# -*- coding: utf-8 -*-
"""
This code do elastix registration and compare landmarks distance after registration
...
def do_elastix(elastix_exe, fixed, moving, output_directory, elastix_affine_pars, elastix_bspline_pars, fixed_mask='',
               moving_mask=''):
    '''
    ...
    # from elastix manual: A fixed image mask is sufficient to focus the registration on a ROI, since samples are drawn from the fixed image. You only want to use a mask for
    ...
    if not os.path.exists(output_warped_image_elastix):
        ...
        # replace "TransformParameters.0.txt" to basename_fixed_moving+'_TransformParameters.0.txt', since TransformParameters.1.txt will read basename_fixed_moving+'_TransformParameters.0.txt
...
if with_mask:
    output_directory = r'D:\15patienets30exames_landmark_registration_project\results\elastix_transformRigidityPenaltydefault0.3_with_mask'
else:
    output_directory = r'D:\15patienets30exames_landmark_registration_project\results\elastix_transformRigidityPenaltydefault0.3'
    # output_directory = r'D:\15patienets30exames_landmark_registration_project\results\elastix_transformRigidityPenalty0.0'
...
# elastix_bspline_pars=current_path + os.sep + 'elastix_pars_bspline_transformRigidityPenalty0.0.txt'
