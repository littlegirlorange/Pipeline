...
from BreastCAD import dcm2mha, rf_segmentation, ann_segmentation, elastix_registration, transformix_registration, \
                      motion_correction, threshold_image, remove_vessels, image_subtraction
...
def main():
    ...
    for iItem, item in enumerate(tasklist):
        ...
        if DCM_INPUT_DIRECTORY:
            _inputDir = DCM_INPUT_DIRECTORY + os.sep + _study_no
            if not os.path.exists(_inputDir):
                print "ERROR: DICOM data not found (" + _inputDir + "). Skipping."
                continue
            for ano in _accession_nos:
                if not os.path.exists(_inputDir + os.sep + ano):
                    print "ERROR: Accession number not found (" + ano + "). Skipping."
                    continue

        # Set output directory for this patient.
        ...
        #
        print "  Converting DICOM to MHA for all studies..."
        bDoConversion = True
        if bOutdirExists:
        ...
        if bDoConversion:
            ...
            for ano in _accession_nos:
                ...
                if len(contents) < 6:
                    print "    ERROR in DICOM conversion: Missing one or more DICOM series for accession no. {0}. Skipping.".format(ano)
                    continue
                ret_val = dcm2mha.do_dcm2mha(DCM23D_EXE, indir, _outputDir, DCM23D_DICOM_FILE_FILTER, DCM23D_SERIES_DESC_FILTER)
                if ret_val > 0:
                    print "    ERROR in dcm2mha: Skipping patient."
                    continue

            # Make sure all required series were converted.
            contents = os.listdir(_outputDir)
            matches = fnmatch.filter(contents, _study_no+"*"+MOTIONCORRECTION_FIXED_IMG_TYPE_FILTER)
            if len(matches) != 3:
        ...
        #  suppressed image).
        #
        print "  Motion correcting all studies..."
        # Find pre- and post-contrast-enhanced input files.
        contents = os.listdir(_outputDir)
        for ano in _accession_nos:
            matches = fnmatch.filter(contents, _study_no+"*"+ano+"*"+MOTIONCORRECTION_FIXED_IMG_TYPE_FILTER)
            fixed_file = os.path.join(_outputDir, matches[0])
            matches = fnmatch.filter(contents, _study_no+"*"+ano+"*"+MOTIONCORRECTION_MOVING_IMG_TYPE_FILTER)
            moving_files = []
            for match in matches:
                moving_files.append(os.path.join(_outputDir, match))

            ret_val = motion_correction.do_motion_correction(MOTIONCORRECTION_EXE, fixed_file, moving_files)
            if ret_val != 0:
                print "    ERROR running motionCorrection.exe: Skipping patient."
                continue

        # --------------------------------------------------------------------------------------------------------------
        #  Calculate subtraction images.
        #
        print "  Calculating subtraction images for all studies..."
        contents = os.listdir(_outputDir)
        for ano in _accession_nos:
        ...
        #
        print "  Calculating breast masks for non-fat suppressed images for all studies..."
        for path, dirnames, filenames in os.walk(_outputDir):
            ...
            for f in files:
                ...
                ret_val = rf_segmentation.do_rf_segmentation(RFSEGMENTATION_EXE,
                                                   ...
                                                   outputFile)
                if ret_val != 0:
                    print "    WARNING breast mask failed for {0}.".format(f)

        # --------------------------------------------------------------------------------------------------------------
        #  Perform ANN lesion segmentation.
        #
        bRunANN = False
        if bRunANN:
            print "  Finding lesions in all studies..."
            # Find pre and contrast-enhanced input files and mask file.
            contents = os.listdir(_outputDir)
            for ano in _accession_nos:
                matches = fnmatch.filter(contents, _study_no+"*"+ano+"*"+ANN_PRECONTRAST_IMG_TYPE_FILTER)
                input_files = []
                if len(matches) != 1:
                    print "    ERROR: Incorrect number of precontrast images found. Exiting."
                    continue
                input_files.append(os.path.join(_outputDir, matches[0]))
                matches = fnmatch.filter(contents, _study_no+"*"+ano+"*"+ANN_POSTCONTRAST_IMG_TYPE_FILTER)
                if len(matches) != 4:
                    print "    ERROR: Incorrect number of postcontrast images found. Exiting."
                    continue
                for match in matches:
                    input_files.append(os.path.join(_outputDir, match))
                matches = fnmatch.filter(contents, _study_no+"*"+ano+"*"+RFSEGMENTATION_OUTPUT_FILE_POSTFIX)
                if len(matches) != 1:
        ...
        # --------------------------------------------------------------------------------------------------------------
        #  Register prior studies to most recent study.
        #
        print "  Registering studies..."
        # Find fixed and moving series to register.
        ...
        for _accession_no_moving in _accession_nos_moving:
            ...
            # Run Elastix.
            print "    Running Elastix on fixed ({0}), moving ({1}).".format(_accession_no_fixed, _accession_no_moving)
            ret_val = elastix_registration.do_elastix(ELASTIX_EXE, fixedFile, movingFile, _outputDir,
                                                      ELASTIX_AFFINE_PAR_FILE, ELASTIX_BSPLINE_PAR_FILE,
                                                      fixedMaskFile, movingMaskFile)
            if ret_val != 0:
                print "      ERROR: Elastix failed. Skipping."
                continue
            ...
            # --------------------------------------------------------------------------------------------------------------
            #  Move all past series for this study using Transformix.
            #
            ...
            for filter in TRANSFORMIX_IMG_TYPE_FILTER:
                ...
                for match in matches:
                    # Run Transformix.
                    print "    Transforming {0}".format(match)
                    ...
                    if ret_val != 0:
                        print "      ERROR: Transformix failed. Skipping."
                        continue

            # --------------------------------------------------------------------------------------------------------------
            #  Remove vessels from lesion probability map images.
            #
            bRemoveVessels = False
            if bRemoveVessels:
                ...
                for ano in (_accession_no_fixed, _accession_no_moving):
                    for filter in THRESHOLD_IMG_TYPE_FILTER:
                        matches = fnmatch.filter(contents, _study_no+"*"+ano+"*"+filter)
